...
void Game::Init()
{
	...
	for (auto &tank : tanks) 
	{
        tank_ptrs.emplace_back(&tank);
        grid.add(&tank);
	}

    particle_beams.push_back(ParticleBeam(vec2(SCRWIDTH / 2, SCRHEIGHT / 2), vec2(100, 50), &particle_beam_sprite, PARTICLE_BEAM_HIT_VALUE));
    particle_beams.push_back(ParticleBeam(vec2(80, 80), vec2(100, 50), &particle_beam_sprite, PARTICLE_BEAM_HIT_VALUE));
    particle_beams.push_back(ParticleBeam(vec2(1200, 600), vec2(100, 50), &particle_beam_sprite, PARTICLE_BEAM_HIT_VALUE));
}
...
{
    //thread t1(&Grid::handleCollisions, &grid);
    //grid.handleCollisions();
    thread_pool.enqueue([]() { grid.handleCollisions(); });

    //Update tanks
    //auto t1 = std::chrono::high_resolution_clock::now();

	thread_pool.enqueue/*thread tt*/([&]() {
    for (Tank &tank : tanks)
    {
            if (tank.active)
            {
    ...
    for (ParticleBeam& particle_beam : particle_beams)
    {
...
void Game::Draw()
{
    ...
    for (int t = 0; t < 2; t++)
    {
        ...
        //std::vector<Tank*> sorted_tanks = merge_sort.mergeSort(tank_ptrs);
        ...
        for (int i = 0; i < NUM_TANKS; i++)
        {
            ...
            screen->Bar(health_bar_start_x, health_bar_start_y + (int)((double)HEALTH_BAR_HEIGHT * (1 - ((double)tank_ptrs.at(i)->health / (double)TANK_MAX_HEALTH))), health_bar_end_x, health_bar_end_y, GREENMASK);
        }
    }
}
